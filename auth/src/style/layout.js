...
function order3Columns (items) {
  ...
  for (let i=0; i<items.length; i++) {
    if (col1 <= col2) {
      if (col1 <= col3) {
        items[i].order = "-1";
        col1 = col1 + items[i].height/(items[i].width/contWidth) + 80; }
      else {
        items[i].order = "1";
      ...
      if (col2 <= col3) {
        items[i].order = "0";
        col2 = col2 + items[i].height/(items[i].width/contWidth) + 80; }
      else {
        items[i].order = "1";
        col3 = col3 + items[i].height/(items[i].width/contWidth) + 80; }
    }
...
function order1Column (items) {
  ...
  let height = 0;
  for (let i=0; i<items.length; i++) {
    height = height + items[i].height/(items[i].width/contWidth) + 80;
  }
  $(".elements-gride").height(height);
  return items;
}
